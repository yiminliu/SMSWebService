package com.tscp.mvno.smpp.util.mail;
...
public class MailClient {
  ...
  public static final Recipient RECIPIENT_SUPPORT = new Recipient("SmsGateway", "Admin", "", "yliu@telscape.net");
  ...
  public void postMail(Vector<Recipient> recipients, String subject, String message, String from) throws MessagingException {
    ...
    try {
      addressFrom.setPersonal("SmsGateway");
    ...
    msg.addHeader("SMSAlert", "SMSAlert");
    ...
    try {
      ...
      if (validVectorAddressList == null || validVectorAddressList.isEmpty()) {
        Recipient lRecipient = new Recipient(null, null, null, "yliu@telscape.net");
        validVectorAddressList.add(lRecipient);
        String recipientList = "Exception Message :: " + send_ex.getMessage() + "\n\n";
        recipientList += "Unaltered Recipient List :: \n";
        for (int i = 0; i < recipients.size(); ++i) {
          if (recipients.get(i) instanceof Recipient) {
  ...
  public static void sendHTML(String from, String to, String subject, String body) throws Exception {
