	...
	public SMPPResponse sendRequest(Connection smppConnection, SubmitSM shortMsg) throws SmsException{
	    ...
	    try {
		   response = smppConnection.sendRequest(shortMsg);
	    	...
	    	throw new SmsIOException(this, "sendRequest", shortMsg.getDestination().getAddress(), "SocketException occured while sending request", ste);
	    }
	    catch(IOException ioe){
	    	throw new SmsIOException(this, "sendRequest", shortMsg.getDestination().getAddress(), "IOException occured while sending request", ioe);
	    }
	    return response;
	}
